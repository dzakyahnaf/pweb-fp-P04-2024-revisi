<template>
  ...
  >
    ...
    >
      ...
      >
        <h2 class="text-2xl font-bold mb-6 text-gray-800">Laporan Fasilitas</h2>
        <table
        ...
        >
          <thead>
            <tr class="border-b-2 border-gray-400">
              ...
              <th class="px-4 py-2 font-bold">Penghuni</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(item, index) in report" :key="index" class="border-b">
              ...
              <td class="px-4 py-2">
                {{ item.user_details[0]?.username || "Unknown" }}
              </td>
            </tr>
          </tbody>
        </table>
...
const getLaporan = async () => {
  try {
    const response = await fetch(
      "http://localhost:3001/admin/laporan/fasilitas",
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      return response.json().then((errorData) => {
        ...
        throw new Error("Failed to fetch data fasilitas");
      });
    }
